...
内嵌的 sing-box 来自 [`Veyra Core`](https://github.com/Zayvian/veyra-core)，一个带
...
`users.reset_day`、`reset_hour`、`reset_minute` 是每月已用流量归零的本地日期时间，0 表示不重置。月付套餐用的：否则运营者
...
- **比对的是上次归零时间，不是存一个「下次到期」。** 面板在某人的重置日当天没开机，
  下次启动时会补上，而不是丢掉这个月。
- **手动清零也会打上时间戳**，否则运营者刚手动清完，下一分钟的定时任务又清一遍。
...
- **给一个已有账号打开重置，也从打开的那一刻开始计周期。** 否则一个跑了半年的账号会被
  ...
  整个面板清空一次。
- **同一时刻到期优先。** 已到期账号不因重置恢复；归零的只是计数器 —— 概览页的曲线和
  按节点的历史流量账都来自 `traffic` 表，不受影响。
- 归零可能让一个超限的账号重新可用，所以要推给节点 —— 吊销是靠从推送里省略实现的，
  恢复也一样。

面板每分钟跑一次，启动时立刻跑一次。节点的本周期统计使用相同的日期时间规则，但只清
节点自己的周期计数，不会删除历史账或影响用户额度。
...
管理员可把浏览器 UI 挂在同域名的自定义路径（例如 `/skypanel`）。设置保存在 SQLite，因此安装器更新、二进制重启和域名证书续签都不会把路径改回根目录。路径外的浏览器 UI 路由返回 404，根路径可选跳转到外部正常网页；这降低了随手扫描看到登录页的机会，但不是认证机制，登录会话、CSRF 和限流仍是实际保护。

节点 WebSocket `/api/v1/node/connect` 和订阅 `/sub/<token>` 固定在根路径，不随管理路径移动。前者已由独立 node token 认证，后者的 token 本身是凭据；保持地址稳定意味着修改管理路径不会中断现有节点或订阅。模板中的管理链接和 HTMX 请求统一经 `panelPath` 生成，避免部分按钮回到根路径而暴露或失效。

### 11.0a 强制全员更新订阅

全员更新不是轮换 `sub_token`：那会让用户保存的旧 URL 变成 404，反而无法把新配置交付给用户。操作只轮换 `vless_uuid`、通用协议密码和 SS2022 用户 PSK，并在同一个数据库事务中记下操作时间、操作人和用户数量；配额、到期、入站权限、流量记录及 `sub_token` 都不改。

普通用户变化只走 `UsersChanged`，以免日常编辑断开连接。这里刻意向每个节点发 `ConfigChanged`：节点会重建监听器并取得新用户表，现有认证流随之关闭，旧配置不能靠既有长连接继续使用。订阅 URL 仍可返回新凭据，因此用户在原客户端执行更新订阅即可恢复。节点离线时不会服务流量，重新接入后会收到最新配置。
