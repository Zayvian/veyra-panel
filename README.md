...
7. [更新与旧版迁移](#7-更新与旧版迁移)
8. [卸载、彻底清理与重装](#8-卸载彻底清理与重装)
9. [日常检查和故障排查](#9-日常检查和故障排查)
...
从旧版升级到 Veyra 时，安装器会继续使用已有的 `/opt/skysbx`、`skysbx-panel.service` 和 `skysbx-node.service`，以保留数据库、节点 token 和证书；这些是兼容标识，不会出现在网页品牌或新的安装链接中。
...
首次安装和旧版升级后仍可从 `https://panel.example.com/login` 登录。登录后进入导航栏的「设置」，在「面板访问路径」填入例如 `/skypanel`，保存后管理页就改为：

```text
https://panel.example.com/skypanel
```

此时根域名可选择跳转到一个正常网页；留空则根域名返回 404。之后所有管理页面、登录、退出和编辑操作都必须带该路径。节点控制连接 `/api/v1/node/connect` 与订阅 `/sub/<token>` 保持原地址，因此改路径不会让节点离线或让已发出的订阅失效。需要改名或恢复根路径时，从「设置」直接修改；保存后浏览器会自动跳到新地址。
...
1. 填到期日期时间、流量上限、同时在线 IP 限制；时间按面板服务器本地时区计算，空额度或 `0` 表示不限流量。
2. 套餐 200 G，在「流量上限 GiB」填 `200`。本项目一个套餐单位按 1024³ 字节计算，订阅 GB 状态使用相同换算。
3. 月度套餐选择流量重置日和时间；不重置表示持续累计。到期与重置设为同一时刻时，到期优先，重置不会延长到期时间。
...
当迁移节点、下线机器或怀疑订阅内容已泄露时，进入「设置 → 强制全员更新订阅」，输入确认文字 `全部断连` 后执行。面板会为**所有用户**重新生成 VLESS、AnyTLS、Hysteria2、TUIC 和 Shadowsocks 2022 的连接凭据，并通知已登记节点重建监听；旧配置立即失效，已有代理连接会断开。

每个用户的订阅 URL 不会改变，也不会清除流量、到期时间、可用入站或用户状态。用户只需在客户端对原订阅执行「更新订阅」即可拿到新配置并恢复使用。离线节点会在下次接入面板时获得新配置；其离线期间本来就无法提供服务。
...
**节点概览周期**：在「节点 → 编辑」可给每台节点单独设置每月统计重置日和时间。概览页显示该节点本周期上行、下行、总量和下次重置；重置不会删除历史流量，也不影响用户套餐或倍率扣量。
